...
import { confirmEmail } from "@/lib/user-api";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setState("success");
        setMessage(result.message || "Your email was confirmed successfully.");
        notifyAuthChanged();
      } catch (err) {
        if (cancelled) return;
        setState("error");
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-background">
      ...
      <div className="w-full max-w-md glass-panel rounded-2xl p-6 space-y-6">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Confirm email</h1>
