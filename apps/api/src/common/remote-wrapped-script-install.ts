...
export function buildRemoteEnvAndWrappedShInstallScript(options: {
  ...
  if (options.runUserScriptOnHostViaDockerSocket === true) {
    innerShLines.push(
      ...
      '# `docker run` does not inherit the host shell env: re-source the same .env inside the helper',
      '# container so `chroot /host bash` sees WEEHAWK_* (on-host build, clone URL, etc.).',
      '# Use host networking: chroot only switches the filesystem root; the process still used the',
      '# bridge network namespace before, so `git clone` / `curl` could not resolve DNS like the host.',
      'if ! command -v docker >/dev/null 2>&1; then',
      '  echo "docker CLI not found; cannot run webhook script on host." >&2',
      '  exit 127',
      'fi',
      'docker run --rm -i --network host -v /:/host -e "ENV_FILE=$ENV_FILE" alpine:3.20 sh -euo pipefail -c \'',
