...
/**
 * Rows owned by an organization only (no {@link OrganizationMembership} user id on the entity).
 * Access: user must be a member of the row's {@link TEntity#organizationId}.
 */
export class OrganizationResourceScopedRepository<
  TEntity extends ObjectLiteral & {
    id?: number;
    organizationId: number;
  },
