...
export function buildTraefikStaticYaml(opts: {
  ...
  const hasPlatform = Boolean(opts.platformDomain?.trim());
  const lines = [
 ...
 */
export function buildTraefikStackYaml(opts: {
  dockerNetwork: string;
  platformDomain?: string;
}): string {
  const { dockerNetwork } = opts;
...
/**
 * Provision-time platform dynamic config (Traefik file provider).
 *
 * Routing layout (mirrors the reference Swarm labels in deployment docs):
 *  - `/api/ws/*`               → API (stripPrefix `/api`, so it reaches `/ws/*` on :8080)
 *  - `/api/*` (excluding ws)   → Web (Next.js proxy adds `X-Weehawk-Api-Key` and same-origin cookies)
 *  - `Host(domain)`            → Web UI
 *
 * Sending `/api/*` straight to the API bypasses the Next proxy that injects the
 * shared API key, which is exactly why "Invalid or missing API key" appeared on
 * public deployments behind Traefik.
 */
export function buildProvisionPlatformDynamicYaml(platformDomain: string): string {
  const host = platformDomain.trim().toLowerCase();
  if (!host) return '';
...
/**
 * Detect the weehawk application stack name (e.g. `weehawk-prod`).
 * Bash snippet: sets `WH_STACK` or leaves it empty.
 */
const BASH_DETECT_WEEHAWK_STACK = `
WH_STACK=""
for s in $($SUDO_CMD docker stack ls --format '{{.Name}}' 2>/dev/null); do
  if $SUDO_CMD docker stack services "$s" --format '{{.Name}}' 2>/dev/null | grep -q '_api$'; then
    if $SUDO_CMD docker stack services "$s" --format '{{.Name}}' 2>/dev/null | grep -q '_web$'; then
      WH_STACK="$s"
      break
    fi
  fi
...
/**
 * Bash: update CORS_ORIGIN on the api/cron-worker/provision-worker services to include the domain.
 * Reads current CORS_ORIGIN, appends domain origins if missing, then `docker service update`.
 */
function buildAppStackDomainUpdateBash(platformDomain: string): string {
  const domain = platformDomain.trim();
  if (!domain) return '';
  const domainOrigins = `https://${domain},http://${domain}`;
  return `
# --- Update application stack env vars for domain: ${domain} ---
${BASH_DETECT_WEEHAWK_STACK}
if [ -z "$WH_STACK" ]; then
  echo "Weehawk: could not detect application stack (no stack with _api + _web services). Skipping env update."
else
  echo "Weehawk: detected application stack: $WH_STACK"
  DOMAIN_ORIGINS="${domainOrigins}"
  DOMAIN_URL="https://${domain}"

  weehawk_service_update_env() {
    local svc="$1"
    shift
    if $SUDO_CMD docker service inspect "$svc" >/dev/null 2>&1; then
      echo "Updating $svc ..."
      $SUDO_CMD docker service update --detach=true "$@" "$svc" >/dev/null 2>&1 || echo "WARN: failed to update $svc"
    fi
  }

  # Read current CORS_ORIGIN from the api service and append domain origins
  CURRENT_CORS=$($SUDO_CMD docker service inspect "\${WH_STACK}_api" --format '{{range .Spec.TaskTemplate.ContainerSpec.Env}}{{println .}}{{end}}' 2>/dev/null | grep '^CORS_ORIGIN=' | head -1 | sed 's/^CORS_ORIGIN=//')
  if [ -n "$CURRENT_CORS" ]; then
    if echo "$CURRENT_CORS" | grep -qF "${domain}"; then
      NEW_CORS="$CURRENT_CORS"
    else
      NEW_CORS="\${CURRENT_CORS},\${DOMAIN_ORIGINS}"
    fi
  else
    NEW_CORS="$DOMAIN_ORIGINS"
  fi

  for svc_suffix in api cron-worker provision-worker; do
    weehawk_service_update_env "\${WH_STACK}_\${svc_suffix}" \\
      --env-add "CORS_ORIGIN=\${NEW_CORS}" \\
      --env-add "WEBFRONTEND_BASE_URL=\${DOMAIN_URL}"
  done

  weehawk_service_update_env "\${WH_STACK}_web" \\
    --env-add "NEXT_PUBLIC_API_URL=\${DOMAIN_URL}"

  echo "Weehawk: application services updated with domain $DOMAIN_URL"
  echo "Weehawk: services will rolling-restart. This may take a moment."
fi`;
}

/**
 * Lightweight bash: only rewrites the Traefik static config and redeploys the Traefik stack.
 * Use after changing the ACME email, platform domain, or other Traefik settings on an already-provisioned host.
 * When platformDomain is set, also updates the application stack env vars (CORS, WEBFRONTEND_BASE_URL, NEXT_PUBLIC_API_URL).
 */
export function buildTraefikRedeployScript(opts: {
  acmeEmail: string;
  platformDomain?: string;
  ...
  const net =
    ...
    'weehawk';
  const pd = (opts.platformDomain ?? '').trim();
  const traefikStaticYaml = buildTraefikStaticYaml({
    acmeEmail: (opts.acmeEmail ?? '').trim(),
    dockerNetwork: net,
    platformDomain: pd,
  });
  const traefikStackYaml = buildTraefikStackYaml({
    dockerNetwork: net,
    platformDomain: pd,
  });
  ...
  const dynamicDir = WEEHAWK_TRAEFIK_DYNAMIC_HOST_PATH;
  let dynamicBashBlock: string;
  if (pd) {
    const dynamicYaml = buildProvisionPlatformDynamicYaml(pd);
    const dynamicB64 = Buffer.from(dynamicYaml, 'utf8').toString('base64');
    dynamicBashBlock = `
DYNAMIC_DIR="${dynamicDir}"
$SUDO_CMD mkdir -p "$DYNAMIC_DIR"
printf '%s' '${dynamicB64}' | base64 -d | $SUDO_CMD tee "$DYNAMIC_DIR/weehawk-platform.yml" > /dev/null
...
if [ -f "$DYNAMIC_DIR/weehawk-platform.yml" ]; then
  ...
  const appStackUpdateBash = buildAppStackDomainUpdateBash(pd);

  return `
set -e
echo "Weehawk: Traefik config redeploy — no Docker/Swarm changes."
...
export function buildWeehawkProvisionScript(opts: {
  ...
  acmeEmail?: string;
  /** Platform domain for Traefik file-provider routing to the Weehawk UI. */
  platformDomain?: string;
}): string {
  const net =
    (opts.overlayNetworkName ?? WEEHAWK_TRAEFIK_EXTERNAL_NETWORK).trim() ||
...
`.trim();
  ...
  const dynamicDir = WEEHAWK_TRAEFIK_DYNAMIC_HOST_PATH;
  let provisionPlatformBash: string;
  if (pd) {
    const dynamicYaml = buildProvisionPlatformDynamicYaml(pd);
    const dynamicB64 = Buffer.from(dynamicYaml, 'utf8').toString('base64');
