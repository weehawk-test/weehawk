/** Engines aligned with DatabaseGeneratorService / database UI */
...
/** Namespaced so validation is unambiguous per engine. */
export type DatabaseBackupFormat =
  | 'postgres_sql_gzip'
...
export const DEFAULT_BACKUP_FORMAT: Record<
...
export function resolveBackupFormat(
  ...
  const d = DEFAULT_BACKUP_FORMAT[engine];
  if (raw == null || String(raw).trim() === '') return d;
  const s = String(raw).trim();
  if (isValidBackupFormatForEngine(engine, s)) return s as DatabaseBackupFormat;
  return d;
}
...
export type DatabaseBackupConfig = {
  engine: DatabaseBackupEngine;
  /** Compose service name to `docker compose exec` into */
  composeService: string;
  /** Logical database name (omit for redis) */
  databaseName?: string;
  /** DB user (postgres default postgres; mysql/mariadb default root) */
  dbUser?: string;
  /** Output / tool variant; defaults per engine when omitted */
  backupFormat?: DatabaseBackupFormat;
};

export function assertSafeComposeService(name: string): string {
  const s = name.trim();
...
export function assertSafeDbIdentifier(name: string): string {
  const s = name.trim();
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/.test(s)) {
...
/**
 * Human-readable description of what the server will run (for UI preview).
 */
export function describeDatabaseBackupPreview(c: DatabaseBackupConfig): string {
