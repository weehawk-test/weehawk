...
import {
  Webhook, LayoutDashboard, FolderKanban, KeyRound, Settings, ChevronUp,
  ImageIcon, Box, Database, Bell, HardDrive,
} from "lucide-react";
      ...
      <div className="flex-shrink-0 border-t border-white/5 p-3">
        <AnimatePresence>
          {profileOpen && (
            ...
            >
              <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-white/5 transition-colors">
                <Settings className="w-4 h-4" />Settings
              </button>
              <div className="h-px bg-white/5 my-1" />
              <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-400/80 hover:text-red-400 hover:bg-red-500/5 transition-colors">
        ...
        >
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary/25 to-white/5 border border-primary/20 flex items-center justify-center flex-shrink-0">
            <span className="text-xs font-bold text-primary tracking-tight">{USER.initials}</span>
          </div>
          <div className="flex-1 text-left min-w-0">
            <p className="text-sm font-semibold text-foreground leading-none truncate">{USER.name}</p>
            <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{USER.role}</p>
          </div>
