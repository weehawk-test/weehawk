...
import {
  ...
  Info, KeyRound, Hash, FolderKanban, CheckCircle, XCircle,
  Download, Edit3, Save, X, Calendar, Tag, Plus,
  Terminal, Rocket, RefreshCw, Square, Play, RotateCw, Activity, Loader2,
  Shield, Variable, Globe, ExternalLink, Link2, ScrollText,
} from "lucide-react";
import {
  useService,
  useDeleteService,
...
import { useDockerSecrets } from "@/hooks/use-docker-secrets";
...
import { parseServiceLogsSseData, serviceLogsStreamUrl } from "@/lib/services-api";
...
type Tab = "overview" | "config" | "env" | "domain" | "secrets" | "logs";
...
export default function ServiceDetails() {
  ...
  const { data: secrets } = useDockerSecrets();
  ...
  useEffect(() => {
    ...
    es.onmessage = (ev) => {
      gotData = true;
      const chunk = parseServiceLogsSseData(ev.data);
      if (!chunk) return;
    ...
    es.onerror = () => {
      ...
      if (!gotData) {
        setLiveLogError(
          "Could not open log stream. Check CORS (CORS_ORIGIN includes this app URL) and that containers are running.",
        );
      }
    };
    return () => {
  ...
  // ── Deploy / Reload (Docker API) ──
  const handleRunDocker = (mode: "deploy" | "reload") => {
    if (!service) return;
    setActiveTab("logs");
    deploy.mutate(
      { serviceId: service.id, serviceName: service.name, serviceType: service.type, mode },
      {
        onSuccess: (log) => {
          ...
          toast({
            title: mode === "reload" ? "Reload finished" : "Deploy successful",
            description:
              mode === "reload"
                ? `${service.name} updated without rebuild.`
  ...
  const tabs: { id: Tab; label: string; icon: typeof Info; count?: number }[] = [
    ...
    { id: "secrets",  label: "Secrets", icon: Shield, count: linkedSecrets.length },
        ...
        <div className={`glass-panel rounded-2xl p-6 relative overflow-hidden ${typeConf.glow}`}>
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                type="button"
                onClick={() => handleRunDocker("reload")}
                disabled={actionBusy}
                title="docker compose up -d --no-build (no image rebuild)"
              ...
              >
                {deploy.isPending && deploy.variables?.mode === "reload" ? (
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "overview" && (
            <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <InfoCard icon={service.isActive ? <CheckCircle className="w-4 h-4 text-emerald-400" /> : <XCircle className="w-4 h-4 text-zinc-400" />}
                label="Status" value={service.isActive ? "Active" : "Inactive"}
                badge={service.isActive ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-zinc-500/10 text-zinc-400 border-zinc-500/20"} />
            ...
            <motion.div key="secrets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Docker Swarm secrets available on the host (same list as the{" "}
                  <span className="text-foreground font-medium">Secrets</span> page)
                </p>
                <Link href="/secrets">
                  <button className="flex items-center gap-1.5 text-sm text-primary hover:underline">
                    <KeyRound className="w-3.5 h-3.5" />Manage Secrets
                  </button>
                </Link>
              </div>
              {linkedSecrets.length === 0 ? (
                <div className="glass-panel rounded-2xl p-10 flex flex-col items-center text-center">
                  ...
                  <Link href="/secrets">
                    <button className="btn-primary text-sm flex items-center gap-2">
                      <KeyRound className="w-4 h-4" />Go to Docker Secrets
                    </button>
                  </Link>
                </div>
              ) : (
                <div className="glass-panel rounded-2xl overflow-hidden">
                  <table className="w-full">
                    ...
                    <tbody>
                      {linkedSecrets.map((secret, i) => (
                        <motion.tr key={secret.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: i * 0.04 }}
                          className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                          <td className="py-3.5 px-5"><span className="font-mono text-sm text-primary font-medium">{secret.name}</span></td>
                          <td className="py-3.5 px-5"><span className="text-xs text-muted-foreground">{formatSecretDate(secret.createdAt)}</span></td>
                        </motion.tr>
                      ))}
                    </tbody>
                  </table>
